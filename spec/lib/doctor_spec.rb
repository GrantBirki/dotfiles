...
RSpec.describe Dotfiles::Doctor do
  ...
  it "reports a healthy install" do
    ...
    expect(stdout).to include("Secretive socket:", "Git signing key:", "Git allowed signers:", "Git Secretive signing helper include:", "Secretive agent exposes Git signing key")
  end
  ...
  it "warns when the Git Secretive signing helper include is stale" do
    prepare_secretive_git
    File.write(git_secretive_program_include_path, "stale")

    status, _stdout, stderr = run_doctor(entries: [])

    expect(status).to eq(0)
    expect(stderr).to include("Git Secretive signing helper include is stale")
  end
